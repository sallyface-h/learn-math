...
open class EZPlayer: NSObject {
    ...
    private lazy var videoDownloader: EZVideoDownloader = {
        let view = EZVideoDownloader()
        view.delegate = self
        return view
    }()
    ...
    open var floatInitFrame = CGRect(x: Appearance.screenBounds.size.width - 213 - 10, y: Appearance.screenBounds.size.height - 120 - 49 - 34 - 10, width: 213, height: 120)
    ...
    open func replaceToPlayWithURL(_ url: URL, title: String? = nil) {
        ...
        guard var url = self.contentURL else {
            self.state = .error(.invalidContentURL)
            return
        }
        
        if let cachedUrl = EZVideoCache.shared.getCachedFileURL(forURL: url.absoluteString) {
            url = cachedUrl
            
            (self.controlView as? EZPlayerControlView)?.toolBarContainer.isUserInteractionEnabled = true
            ...
            ProgressHUD.progress("Loading...", 0.0, interaction: true)
            
            (self.controlView as? EZPlayerControlView)?.toolBarContainer.isUserInteractionEnabled = false
            videoDownloader.downloadVideo(from: url.absoluteString)
        }
    }
    ...
    open  func updateCustomView(toDisplayMode: EZPlayerDisplayMode? = nil){
        ...
        case .embedded:
            ...
            if self.playerView?.controlView == nil || self.playerView?.controlView != self.controlViewForEmbedded{
                if self.controlViewForEmbedded == nil {
                    self.controlViewForEmbedded = self.controlViewForFullscreen ?? Configurations.bundle.loadNibNamed(String(describing: EZPlayerControlView.self), owner: self, options: nil)?.last as? EZPlayerControlView
                }
            }
        ...
        case .fullscreen:
            if self.playerView?.controlView == nil || self.playerView?.controlView != self.controlViewForFullscreen{
                if self.controlViewForFullscreen == nil {
                    self.controlViewForFullscreen = self.controlViewForEmbedded ?? Configurations.bundle.loadNibNamed(String(describing: EZPlayerControlView.self), owner: self, options: nil)?.last as? EZPlayerControlView
                }
            }
        ...
        case .float:
            ...
            if self.playerView?.controlView == nil || self.playerView?.controlView != self.controlViewForFloat{
                if self.controlViewForFloat == nil {
                    self.controlViewForFloat = floatModelSupported == .window ?  Configurations.bundle.loadNibNamed(String(describing: EZPlayerWindowView.self), owner: self, options: nil)?.last as? UIView : Configurations.bundle.loadNibNamed(String(describing: EZPlayerControlView.self), owner: self, options: nil)?.last as? EZPlayerControlView
                }
            }
        ...
        case .none:
            ...
            if self.controlView == nil {
                self.controlViewForEmbedded =  Configurations.bundle.loadNibNamed(String(describing: EZPlayerControlView.self), owner: self, options: nil)?.last as? EZPlayerControlView
            }
        }
        self.displayMode = nextDisplayMode
    }
...
// MARK: - EZVideoDownloaderDelegate
extension EZPlayer: EZVideoDownloaderDelegate {
    func ezVideoDownloader(_ downloader: EZVideoDownloader, didUpdateProgress progress: Float, data: Data) {
        ProgressHUD.progress("Loading...", CGFloat(progress), interaction: true)
    }
    
    func ezVideoDownloader(_ downloader: EZVideoDownloader, didFinishDownloadingWith data: Data) {
        (self.controlView as? EZPlayerControlView)?.toolBarContainer.isUserInteractionEnabled = true
        
        guard let url = self.contentURL, !EZVideoCache.shared.hasCachedData(forURL: url.absoluteString) else {
            ProgressHUD.dismiss()
            return
        }
        EZVideoCache.shared.cacheData(data, forURL: url.absoluteString)
        ProgressHUD.dismiss()
        
        prepareToPlay()
        
        if let contentView = self.embeddedContentView {
    ...
    func ezVideoDownloader(_ downloader: EZVideoDownloader, didFailWithError error: Error) {
        (self.controlView as? EZPlayerControlView)?.toolBarContainer.isUserInteractionEnabled = true
        ProgressHUD.error("download failure!!")
    }
}
...
extension EZPlayer {
    private func configFloatVideo(){
        if self.floatContainerRootViewController == nil {
            self.floatContainerRootViewController = EZPlayerWindowContainerRootViewController(nibName: String(describing: EZPlayerWindowContainerRootViewController.self), bundle: Configurations.bundle)
        }
