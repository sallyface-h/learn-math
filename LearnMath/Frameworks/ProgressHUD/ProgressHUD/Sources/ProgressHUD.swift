...
extension ProgressHUD {
	...
	func progress(text: String?, value: CGFloat, interaction: Bool) {
        ...
        setupSizes(text, isInfo: false, false)
		setupNotifications()
		setupPosition()
		displayHUD()
	}
...
extension ProgressHUD {
	...
	func staticImage(text: String?, image: UIImage?, isInfo: Bool, interaction: Bool, delay: TimeInterval?) {
        ...
        setupSizes(text, isInfo: isInfo, false)
		setupNotifications()
		setupPosition()
		displayHUD()
	}
...
extension ProgressHUD {
	...
	func animate(text: String?, interaction: Bool) {
        ...
        setupSizes(text, isInfo: false, true)
		setupNotifications()
		setupPosition()
		displayHUD()
	}
...
extension ProgressHUD {
	...
	private func setupToolbar() {
        ...
        let image = UIImage.image(with: colorHUD)
        toolbarHUD?.setBackgroundImage(image, forToolbarPosition: .any, barMetrics: .default)
	}
}
...
extension ProgressHUD {
	...
	private func setupStaticImage(_ image: UIImage?) {
		if (viewStaticImage == nil && image != nil) {
...
extension ProgressHUD {
    ...
    private func setupSizes(_ text: String?, isInfo: Bool, _ animation: Bool) {
		if let text {
            if isInfo {
                setupSizesTextOnlyWithInfo(text)
            } else {
    ...
    private func setupSizesTextOnlyWithInfo(_ text: String) {
        var rect = rectText(text)

        let width = rect.size.width + 2 * 20.0
        let height = rect.size.height + 2 * 20.0
