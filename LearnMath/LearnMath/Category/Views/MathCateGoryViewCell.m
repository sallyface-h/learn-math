...
-(void)setUpSubViews
{
    _scaleButton = [[ScaleableButton alloc] init];
    ...
    _tutorialLabel.font = [UIFont ap_poppins:14.0 weight:UIFontWeightBold];
    _tutorialLabel.textColor = [UIColor colorForSet:ColorSetPurple];
    [self.tutorStackView addArrangedSubview:_tutorialLabel];
    
    _tutorialButton = [[ScaleableButton alloc] init];
    [self.tutorialButton addTarget:self action:@selector(tutorialButtonAction:) forControlEvents:UIControlEventTouchUpInside];
    ...
    _progressLabel.font = [UIFont ap_poppins:14.0 weight:UIFontWeightBold];
...
- (void)configureWithSkill:(MathSkills *)skill maxTrainCount:(NSInteger)maxTrainCount {
    CGFloat progress = MIN(1.0, (CGFloat)skill.trainCount / (CGFloat)maxTrainCount);
...
- (void)bgButtonAction:(ScaleableButton *)sender {
    if ([self.delegate respondsToSelector:@selector(mathTrainsHomeCell:didClickedBgButton:)]) {
        [self.delegate mathTrainsHomeCell:self didClickedBgButton:sender];
    }
}

- (void)tutorialButtonAction:(ScaleableButton *)sender {
    if ([self.delegate respondsToSelector:@selector(mathTrainsHomeCell:didClickedTutorialButton:)]) {
        [self.delegate mathTrainsHomeCell:self didClickedTutorialButton:sender];
    }
}
@end
