...
- (void)configureWithCalculateNums:(NSArray<NSDictionary *> *)calculateNums
{
    ...
    for (NSInteger i = 0; i < calculateNums.count; i++) {
        NSDictionary *dict = calculateNums[i];
        NSString *text = dict[@"Text"];
        NSString *buttonID = dict[@"ButtonID"];
        ScaleableButton *settingNumButton = [ScaleableButton buttonWithType:UIButtonTypeCustom];
        settingNumButton.accessibilityIdentifier = buttonID;
        ...
        label.font =  [UIFont ap_poppins:14.0 weight:UIFontWeightBold];
